const blob = new Blob(['importScripts("https://theo.monster/services/nopasteweb/worker.js");']);
...
if(!document.getElementById('autoref').checked){event.preventDefault()}
else{setTimeout(refreshPage, 1000)}
});

document.addEventListener('click', function(){
if(!document.getElementById('autoref').checked){event.preventDefault()}
else{setTimeout(refreshPage, 0)}
});
...
const initCodeEditor = () => {
  if (window.location.href.indexOf("?v=page") > -1) {
      document.documentElement.innerHTML = editor.getValue();
    }
    CodeMirror.modeURL = '';
...
const initLangSelector = () => {
    select = new SlimSelect({
        ...
        onChange: (e) => {
            ...
            document.title = e.text && e.text !== 'Plain Text' ? `NoPaste - editting ${e.text} code` : 'NoPaste';
    ...
    select.set(l ? decodeURIComponent(l) : shorten('Markdown'));
};
...
const refresh = (dataToCopy) => {
    window.location.href = dataToCopy;
};
...
const webview = (dataToCopy) => {
    window.open(location.href.replace('nopasteweb', 'web').replace('?l=html', ''));
};
...
const disableautoref = () => {
    byId('disableautoref').classList.add('hidden');
    byId('enableautoref').classList.remove('hidden');
    byId('autoref').click();
};

const enableautoref = () => {
    byId('enableautoref').classList.add('hidden');
    byId('disableautoref').classList.remove('hidden');
    byId('autoref').click();
};

const mdtohtml = () => {
  tomd = new showdown.Converter(),
  editor.setValue(tomd.makeHtml(editor.getValue()));
...
const buildUrl = (rawData, mode) => {
    ...
    if (mode === 'iframe') {
        const height = editor['doc'].height + 60;
