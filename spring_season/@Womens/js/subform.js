...
try {
  if(!lsdciph(localStorage.getItem(brick('$_authenticated_user'+ladderSeason)),0,laddersOffered.Open.ls))  localStorage.setItem(ladderId.ls+akwo, LZString.compress(JSON.stringify(j)));
} catch (ex) {
...
function retainvals() {
    ...
    let lscrds = LZString.decompress(localStorage.getItem(ladderId.ls+akwo));
