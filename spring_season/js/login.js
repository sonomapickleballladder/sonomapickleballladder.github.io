...
ptc('rdbusrs', '*', 'id', 1).then(function(res){
    ...
    if(isvl){
        ...
        if(location.search.indexOf('return_to=')>-1){
        setTimeout(function(){
            alert('Sign in was successful, but the page you are trying to visit will not be accessible until after the Initial Registration Deadline')
            location.replace('dashboard.html')
    },10)   
    
        }
        else{setTimeout(function(){location.replace('dashboard.html')},20)}
...
document.addEventListener("DOMContentLoaded",function() {
    try {
        try{
            if(location.search.indexOf('email=')>-1){byId('email').value=decodeURI(location.search.split('email=')[1].split('&')[0])}
            if(location.search.indexOf('key=')>-1){byId('key').value=decodeURI(location.search.split('key=')[1].split('&auto')[0].split('&return')[0])}
            if(location.search.indexOf('autologin')>-1){authorize({email: byId('email').value, key: byId('key').value})}
            else {hide(0)}
        }catch(ex){hide(0)}
