...
function lgifnolg() { try{if(!lsdciph(localStorage.getItem(ladderId.ls+'logged_in_user'))) {location.replace(`login.html?return_to=${location.pathname}`)} else {try{hide(0)} catch(ex){} return true}}catch(ex){location.replace(`login.html`)}}
function nifnok() { try{if(!lsdciph(localStorage.getItem(brick('$_authenticated_user'+ladderSeason)))) {location.replace(`login.html`)} else {try{hide(0)} catch(ex){} return true}}catch(ex){location.replace(`login.html`)}}
...
function lsWarning(message) {
  ...
  document.addEventListener("DOMContentLoaded",function() {
    try{let lz = LZString.compress('test')}catch(ex){let surl = getScriptURL(); let path = `${getScriptURL().path.split('/')[0]}/lzstring-min.js`, sc = document.createElement('script'); sc.src = path; getScriptURL().script.parentNode.insertBefore(sc, getScriptURL().script)}
...
function get_darkmode() {
  let isdarkmode = null;
try {if(localStorage.getItem(brick('$_darkMode')) != null) {isdarkmode = JSON.parse(localStorage.getItem(brick('$_darkMode')))}} catch(ex) {console.log(ex);}
