function  setrefresh() {
    ...
    function refresh() {
      ...
      try{if(burl.length < 1) { let ms = '<!DOCTYPE html><head></head><pre style="word-wrap: break-word; white-space: pre-wrap;">This page will be available once the Ladder Season is underway.</pre>'; let tt = document.title; document.documentElement.innerHTML = ms; document.title = tt; hide(0); return false}}catch(ex){}
      ...
      try{if(localStorage.getItem(ladderId.ls+'spreadsheet_view')&&Object.keys(urls).includes(localStorage.getItem(ladderId.ls+'spreadsheet_view'))){url = urls[localStorage.getItem(ladderId.ls+'spreadsheet_view')].url}}catch(ex){}
      ...
      for (let i = 0; i < Object.keys(urls).length; i++) {
        if(url == Object.values(urls)[i].url) {view = Object.keys(urls)[i]; try{localStorage.setItem(ladderId.ls+'spreadsheet_view',view)}catch(ex){}}
