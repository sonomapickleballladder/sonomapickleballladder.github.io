...
function postInfo() {
    ...
    if(disallow(name)||localStorage.getItem('unauthorized')) {calert(true,'Error', `You are not authorized ${name}.`, 'error'); localStorage.setItem('unauthorized', true); return false} 
    ...
    //localStorage.setItem(akwo, LZString.compress(JSON.stringify(injson)));
...
try {
    localStorage.setItem(akwo, LZString.compress(JSON.stringify(j)));
} catch (ex) {
...
function retainvals() {
    ...
    let lscrds = localStorage.getItem(akwo);

    if(lscrds) {
        let yg = JSON.parse(LZString.decompress(lscrds));
        //console.log(yg);
        if (yg) {
            try {
