             ...
             if(!res.error) {
              ...
              .then(function(response) {
               ...
               document.body.classList.remove('pulse')
                ...
                if(!response.error) {localStorage.setItem(brick('$_hasRegistered'+ladderSeason), true); document.write('<!DOCTYPE html><pre style="font-size: larger; word-wrap: break-word; white-space: pre-wrap;">Successfully submitted. You should receive a link to confirm your email address shortly. Please check both your inbox and spam folders.</pre>'); document.title = 'Success'}
                else {document.write(`<!DOCTYPE html><pre style="font-size: larger; word-wrap: break-word; white-space: pre-wrap;">An error occurred. Click <a href='mailto:SPLwebsitebugs@riseup.net?subject=Bug Report&body=${sendBug(response.error)}'>here</a> to submit a bug report.</pre>`); document.title = 'Error'}
              })
          }

             else alert('Registration failed. Please make sure you have provided a valid email address, and a password that is at least 6 characters long.')
  ...
  try{
    ...
    if(hasreg) {
      if(location.hash.indexOf('view-responses')<0) {
        document.write('<!DOCTYPE html><pre style="font-size: larger; word-wrap: break-word; white-space: pre-wrap;">You have already signed up. Click <a href="?reload=1#view-responses">here</a> to view your responses.</pre>')
      }
      else {
        if(!svdin) {document.write('<!DOCTYPE html><pre style="font-size: larger; word-wrap: break-word; white-space: pre-wrap;">Something went wrong ☹️</pre>')}
