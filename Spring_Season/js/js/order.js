...
renderorder(wbicewom, plr);
   ...
   byId('order').innerHTML = `<br><span style='opacity: 0'>Initial Order Creation will be initialized ${-frstC} days before the first round starts.</span><br>`
...
function moveDown(element,p1,p0) {
  ...
  async function ptc(t,e,c,n,a,i,r){const{data:o,error:u}=await sb.client.from(init.f_e+t).select(e);u&&debug;try{executeFunctionByName(r,window,o)}catch(t){return o}}
  ...
  function checkid() {
    ...
    try{
      if(!lsdciph(localStorage.getItem(ladderId.ls+'ArprO_lA3RZxD5z'))) {
        location.replace(`login.html?return_to=${location.pathname}`);
      } 
      else {
  ...
  catch(ex){alert('Something went wrong...'); location.replace(`login.html?return_to=${location.pathname}`)}
