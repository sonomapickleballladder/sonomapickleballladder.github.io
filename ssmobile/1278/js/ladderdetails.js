...
function getsignups() {
    ptc('viewable_Signups', '*', 'awx', '1').then(res=>{
        ...
        for (let i = 0; i < ldo.length; i++) {
            ...
            let cdu = '', star=''; try{if(laddersOffered[ldo[i]].min!=0){star='*'; cdu=`, at least ${parseInt(byId(`mns-${ldo[i]}`).innerHTML) - signupsObject[ldo[i]].length} still needed.`}}catch(ex){console.error(ex);}
            ...
            try{byId(`ns-${ldo[i]}`).innerHTML=`${signupsObject[ldo[i]].length==0 ? 'None':signupsObject[ldo[i]].length} so far${cdu}${sci}`}catch(ex){}
