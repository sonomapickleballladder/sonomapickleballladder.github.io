...
function nifnok() { try{if(!lsdciph(localStorage.getItem(brick('$_authenticated_user'+ladderSeason)),0,laddersOffered.Open.ls)) {location.replace(`../login${protoMap[location.protocol]}`)} else {try{hide(0)} catch(ex){} return true}}catch(ex){location.replace(`../login.html`)}}
...
function sendBug(b) {
 ...
 try{ 'object' == typeof b && (b = JSON.stringify(b))
  ...
  let usr = lsdciph(localStorage.getItem(brick('$_authenticated_user'+ladderSeason)),0,laddersOffered.Open.ls)
